...
hvaccycle           = 15 * 60                                       # hvac_update will cycle every 1 minute
...
def update_hvac():
    ...
    if(house_state == 0):               # if the house is closed, run this code
        internal_temp_curr = internal_temp_curr + 0.0495*(external_temp - internal_temp_curr) 
    elif(house_state == 1):             # if a door is open, run this code
        internal_temp_curr = internal_temp_curr + 0.6*(external_temp - internal_temp_curr)
    else:                               # if a window is open, run this code (can be removed if we didn't implement windows)
        internal_temp_curr = internal_temp_curr + 0.3*(external_temp - internal_temp_curr)

    # find delta between internal_temp_curr and the set high and low temperatures
    delta_high = internal_temp_curr - internal_temp_high
    delta_low = internal_temp_low - internal_temp_curr

    # if the delta of either high or low is greater than 0, determine simulated power usage and 
    # set the temperature to 1 degree below or above (respectively) what the temperature is set to
    if(delta_high > 0):
        powerusage = 58.3 + (58.3 * delta_high)
        # TODO put hvac usage into correct table
        internal_temp_curr = internal_temp_curr - 1 - delta_high
    if(delta_low > 0):
        powerusage = 58.3 + (58.3 * delta_low)
        # TODO put hvac usage into correct table 
        internal_temp_curr = internal_temp_curr + 1 + delta_low
